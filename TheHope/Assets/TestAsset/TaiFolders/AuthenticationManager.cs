...
public class AuthenticationManager : MonoBehaviour, IAuthenticationManager
{
	...
	private void Start () {
		...
		ResetPassword("phamf574@gmail.com", "271946", "Adminstrator");
	}
    ...
    public void Register(string email, string password)
    {
        ...
        if (CheckInputInfo(email, password))
        {
            new RegistrationRequest().SetUserName(email).SetPassword(password).SetDisplayName("admin").Send((response) => {
    ...
    public void LoginWithGS(string email, string password)
    {
        new AuthenticationRequest().SetUserName(email).SetPassword(password).Send((response) => {
			...
			Debug.Log("ScriptData " + response.JSONString);
           	 OnLoginSuccess();
        });
    }
    ...
    public void RecoveryAccount(string email)
    {
        ...
        new AuthenticationRequest().SetUserName(email).SetPassword("").SetScriptData(scriptData).Send((response) => {
            if (response.HasErrors)
            {	
				Debug.Log(((GSData) response.JSONData["error"]).GetString("action"));
    ...
    public void ResetPassword (string email, string token, string newPassword)
    {
		...
		new AuthenticationRequest().SetUserName(email).SetPassword(newPassword).SetScriptData(scriptData).Send((response) => {
			if (response.HasErrors) {
				Debug.Log(((GSData) response.JSONData["error"]).GetString("action"));
				return;
			}
